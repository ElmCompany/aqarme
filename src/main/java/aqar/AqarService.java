...
class AqarService {
    ...
    private boolean matchesPrice(JobElement je) {
        try {
            int price = (int) (parseFloat(extractNumber(je.element().select(".label ,radius").text())) * ONE_K);
            boolean ret = true;
            if (je.maxPrice() != null) {
    ...
    private Point toPoint(String url) {
        float latitude = parseFloat(url.substring(url.indexOf(LOC) + LOC.length(), url.indexOf(PLUS)));
        float longitude = parseFloat(url.substring(url.indexOf(PLUS) + PLUS.length()));
